...
def remove_conversation(request):
    ...
    if not session_id:
        return HttpResponseBadRequest('session_id is requreid!')
    
    try:
        session = ChatSession.objects.get(session_id=session_id)
    except ChatSession.DoesNotExist:
        return HttpResponseNotFound('Session is not found')
...
def get_conversation_messages(request):
    ...
    chat_messages = ChatMessage.objects.filter(session=session).order_by('created_at', 'id')
